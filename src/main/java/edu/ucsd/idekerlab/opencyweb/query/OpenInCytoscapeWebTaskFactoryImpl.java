...
public class OpenInCytoscapeWebTaskFactoryImpl extends AbstractTaskFactory implements NetworkCollectionTaskFactory {
	...
	private static final String CYTOSCAPE_WEB_URL_TEMPLATE = "https://web.cytoscape.org?import=http://localhost:1234/v1/networks/${network_suid}.cx?version=2";
	...
	private TaskIterator createTaskIterator(CyNetworkView networkView) {
		doTask = new DoTask(swingApplication, dialogUtil, 
			deskTopUtil, networkView.getModel(), buildCytoscapeWebURI(appManager.getCurrentNetwork().getSUID()));
